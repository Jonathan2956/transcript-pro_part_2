...
class VideoPlayer {
    ...
    async getVideoInfo() {
        ...
        try {
            // ✅ CHANGED: Use dynamic backend URL
            const apiUrl = window.API_BASE_URL + `/youtube/video/${this.currentVideoId}`;
    ...
    async getCaptions(language = 'en') {
        ...
        try {
            // ✅ CHANGED: Use dynamic backend URL
            const apiUrl = window.API_BASE_URL + `/youtube/captions/${this.currentVideoId}`;
