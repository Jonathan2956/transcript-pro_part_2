...
class AIService {
    constructor() {
        this.baseURL = window.API_BASE_URL + '/ai'; // ✅ CHANGED LINE
        this.cache = new Map();
        this.requestQueue = [];
    ...
    async processTranscript(transcriptEntries, videoId, language = 'en') {
        ...
        try {
            const response = await this.makeRequest('/process-transcript', {
                method: 'POST',
                headers: {
    ...
    async fixTranscript(transcriptText, language = 'en', fixType = 'both') {
        try {
            const response = await this.makeRequest('/fix-transcript', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.getAuthToken()}`
    ...
    async analyzePhrases(sentenceText, sentenceId, context = '') {
        ...
        try {
            const response = await this.makeRequest('/analyze-phrases', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.getAuthToken()}`
    ...
    async analyzeComplexity(sentenceText) {
        ...
        try {
            const response = await this.makeRequest('/analyze-complexity', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.getAuthToken()}`
    ...
    async getLearningInsights(sentences, videoId) {
        ...
        try {
            const response = await this.makeRequest('/learning-insights', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.getAuthToken()}`
    ...
    async makeRequest(endpoint, options) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                const url = this.baseURL + endpoint; // ✅ CHANGED: Use baseURL + endpoint
